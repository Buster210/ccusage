...
pub(crate) fn load_entries_from_directory(
    ...
    // then ALWAYS pass them through the ledger under a dedicated namespace so
    // spend is retained even after the whole `opencode.db` is deleted. Skipping
    // the merge on a gone/corrupt db would drop all previously-ledgered spend.
    // `merge_ledger` only re-emits keys ABSENT from the live set, so a present
    // source is never double-counted; `live_only` suppresses re-emit when set.
...
fn fallback_from_cache(
    ...
    let mut entries: Vec<_> = cached
        .into_values()
        .map(|row| crate::LoadedEntry::from(row.entry))
        .collect();
...
/// Load all OpenCode message-database rows, reusing the per-database row cache
/// so unchanged rows skip the expensive JSON parse.
fn load_entries_from_database(
    ...
    // Load the row cache from cache.db FIRST — it survives opencode.db corruption.
    ...
    // Try to open and query opencode.db. On failure, fall back to cached entries
    // so spend is preserved rather than zeroed (corruption) or doubled (ledger
    // re-emission of an empty live set).
...
mod tests {
    ...
    /// Corruption in opencode.db must not zero or double spend — cached rows
    /// are the fallback.
    #[test]
    fn garbage_db_preserves_cached_spend() {
        let _env = CacheEnv::new("garbage-db");
        let fixture = fs_fixture!({});
        let db = fixture.path("opencode.db");
        create_db_message(
            &db,
    ...
    /// Mid-scan read error must not overwrite a good row-cache with truncated
    /// data.  A DB that opens and prepares but fails during iteration triggers
    /// the `!completed` path: fall back to cached entries, skip cache save.
    /// NOTE: SQLite is resilient to page-level corruption and may return Done
    /// instead of Err on a truncated file, so this test uses aggressive
    /// corruption (zeroed data pages) to maximise the chance of a step error.
    #[test]
    fn mid_scan_error_preserves_row_cache() {
        let _env = CacheEnv::new("mid-scan-error");
        let fixture = fs_fixture!({});
        let db = fixture.path("opencode.db");
